...
def missed_tasks():
    ...
    list_tasks_missed = session.query(Table.task, Table.deadline). \
        order_by(Table.deadline).filter(Table.deadline < today).all()

    if len(list_tasks_missed) == 0:
        print('Nothing is missed!')
    else:
        for task in range(len(list_tasks_missed)):
            print(f'{task + 1}. {list_tasks_missed[task][0]}. '
                  f'{list_tasks_missed[task][1].strftime("%d %b").lstrip("0")}')
        print()
...
def delete_task():
    ...
    list_all_tasks = session.query(Table.task, Table.deadline).order_by(Table.deadline).all()

    if len(list_all_tasks) == 0:
        print('Nothing to delete')
    else:
        for every_task in range(len(list_all_tasks)):
            print(f'{every_task + 1}. {list_all_tasks[every_task][0]}. '
                  f'{list_all_tasks[every_task][1].strftime("%d %b").lstrip("0")}')

        del_number = int(input())
        session.query(Table).filter(Table.id == del_number).delete()
        session.commit()
